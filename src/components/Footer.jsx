// import { logoLight, paymentMethod } from "../assets";
import {
  FaFacebookF,
  FaTwitter,
  FaInstagram,
  FaYoutube,
  FaHome,
} from "react-icons/fa";
import { ImGithub } from "react-icons/im";
import { BsPaypal, BsPersonFill } from "react-icons/bs";
import { MdLocationOn } from "react-icons/md";

const Footer = () => {
  return (
    <div className='w-full  h-full mx-auto   bg-black text-white py-20  font-titleFont'>
      <div className=' max-w-[1300px] mx-auto  flex items-start justify-between gap-5'>
        {/* ================= LogoIcon Start here ================== */}
        <div className='flex flex-col gap-7'>
          {/* <img src={logoLight} className='w-[130px]' alt='logoLight' /> */}
          <p className='text-white text-sm tracking-wide'>&copy; ANB FASHION</p>
          {/* <img src={paymentMethod} className='w-[224px]' alt='paymentLogo' /> */}
          <div className='flex text-4 gap-5 font-{25px} '>
            <ImGithub className='hoverStyle' />
            <FaYoutube className='hoverStyle' />
            <FaFacebookF className='hoverStyle' />
            <FaTwitter className='hoverStyle' />
            <FaInstagram className='hoverStyle' />
          </div>
        </div>
        {/* =================  LogoIcon End here ===================  */}

        {/* ================= LocateUs Start here ==================  */}
        <div>
          <h2 className='text-2xl font-semibold text-white mb-4'>Utilisateurs</h2>
          <div className='text-base flex flex-col gap-2'>
            <p>Abidjan</p>
            <p>Phone: 00225 01 40 91 85 87</p>
            <p></p>
          </div>
        </div>
        {/* =================  LocateUs End here ===================  */}

        {/* =================  Profile Start here ==================  */}
        <div>
          <h2 className='text-2xl font-semibold text-white mb-4'>profil</h2>
          <div className='flex flex-col gap-2 text-base'>
            <p className='flex items-center gap-3 hoverStyle'>
              <span>
                <BsPersonFill />
              </span>{" "}
              mon compte
            </p>

            <p className='flex items-center gap-3 hoverStyle'>
              <span>
                <MdLocationOn />
              </span>{" "}
              help & support
            </p>
          </div>
        </div>
        {/* =================  Profile End here ====================  */}

        {/* ================= Subscribe Start here ================= */}
        <form className='flex flex-col gap-3 justify-center'>
          <input
            type='text'
            placeholder='e-mail'
            className='bg-transparent border px-4 py-2 text-sm'
          />
          <button className='text-sm border text-[#8a2be2] border-t-0 hover:bg-gray-900 active:bg-[#F5A623] active:text-black '>
            Subscribe
          </button>
        </form>
        {/* ================= Subscribe End here ===================  */}
      </div>
    </div>
  );
};

export default Footer;
